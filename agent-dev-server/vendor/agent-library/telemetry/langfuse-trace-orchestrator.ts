...
export type RunEndMetadataHook = (params: {
  traceId: string;
  metadata: Record<string, unknown>;
...
type LangfuseTraceOrchestratorParams = {
  ...
  /** Optional hook to transform run-end metadata before it's written to the trace.
   *  Can be async (e.g. to upload large payloads to external storage). */
  onRunEndMetadata?: RunEndMetadataHook;
};

/**
 * Replace large binary tool output with a lightweight summary so Langfuse
 * span.update() doesn't block the event loop serializing multi-MB payloads.
 */
const redactBinaryContent = (output: unknown): unknown => {
  ...
  if (output.type === 'image') {
    return {
      ...
      size: output.image.data.byteLength,
      redacted: true,
    };
  }
  ...
  if (output.type === 'file') {
    return {
      ...
      size: output.file.data.byteLength,
      redacted: true,
    };
  }
  // Summarize parts without copying raw bytes
  return {
    type: 'multi-content',
    description: output.description,
    parts: output.parts.map((part) => {
      if (part.kind === 'text') {
        return { kind: 'text', length: part.text.length };
      }
      if (part.kind === 'image') {
        return {
          kind: 'image',
          ...
          size: part.image.data.byteLength,
        };
      }
      return {
        kind: 'file',
        ...
        size: part.file.data.byteLength,
      };
    }),
    redacted: true,
  };
...
const toUsageBreakdown = (
  ...
  if (typeof usage.inputTokens === 'number' && Number.isFinite(usage.inputTokens)) {
    usageSummary.input = Math.max(0, usage.inputTokens - cacheReadTokens - cacheWriteTokens);
  }
...
/**
 * Wraps a TraceRun so every handler fires in a setTimeout(0) callback.
 * The Langfuse SDK calls JSON.stringify synchronously on every enqueue,
 * which blocks the streaming event loop. This moves that work to the
 * next macrotask. flush() awaits all pending callbacks before sending.
 *
 * setTimeout(0) callbacks are FIFO, so handler ordering is preserved.
 */
const deferTraceRun = (run: TraceRun): TraceRun => {
        ...
        new Promise<void>((resolve) => {
          setTimeout(() => {
            try {
              ...
              // Handler errors are logged internally via logFailure.
            }
            resolve();
          }, 0);
        }),
  ...
  const originalFlush = run.flush;

  return {
    traceId: run.traceId,
    onModelStepStart: wrap(run.onModelStepStart),
    onModelStepEnd: wrap(run.onModelStepEnd),
    onToolStart: wrap(run.onToolStart),
    onToolEnd: wrap(run.onToolEnd),
    onRunEnd: wrap(run.onRunEnd),
    flush: async () => {
...
export class LangfuseTraceOrchestrator implements TraceOrchestrator {
  ...
  private onRunEndMetadata?: RunEndMetadataHook;
  ...
  constructor(params: LangfuseTraceOrchestratorParams) {
    ...
    this.onRunEndMetadata = params.onRunEndMetadata;
  }
  ...
  startRun(params: {
    ...
    return deferTraceRun({
      ...
      onToolEnd: ({ toolCallId, output, error }) => {
        ...
        try {
          span.update?.({
            output: error ? safeString(error) : redactBinaryContent(output),
            endTime: new Date(),
      ...
      onRunEnd: ({ status, output, error, conversationHistory }) => {
        const updateTrace = (metadata: Record<string, unknown>) => {
          trace.update?.({
        ...
        try {
          ...
          if (this.onRunEndMetadata) {
            const metadataWithHistory = { ...metadata, conversationHistory };
            const result = this.onRunEndMetadata({ traceId, metadata: metadataWithHistory });
            if (result instanceof Promise) {
              result
                .then((transformed) => updateTrace(transformed))
                .catch((hookError) => {
                  this.logFailure('onRunEndMetadata', hookError, { traceId, status });
                  updateTrace(metadata);
                });
            } else {
              updateTrace(result);
            }
