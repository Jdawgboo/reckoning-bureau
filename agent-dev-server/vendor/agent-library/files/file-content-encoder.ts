...
export interface FileEncoderConfig {
  /** Enable PDF encoding (requires pdf-parse for text mode) */
  pdf?: boolean;
  /**
   * How `PdfCodec` encodes PDFs. Default `'native'` — emits `base64_pdf`,
   * matches the user-attachment path and lets the model see layout/figures.
   * Use `'text'` for tool-result paths on providers that reject `file-data`
   * in tool_result (notably `@ai-sdk/amazon-bedrock`'s Converse adapter).
   */
  pdfMode?: 'native' | 'text';
...
function buildCodecMaps(config: Required<FileEncoderConfig>): {
  ...
  if (config.pdf) {
    const pdfCodec = new PdfCodec({ mode: config.pdfMode });
