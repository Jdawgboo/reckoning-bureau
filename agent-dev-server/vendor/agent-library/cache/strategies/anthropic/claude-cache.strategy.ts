...
import { COMPACTION_MARKER_PREFIXES } from '../../../defaults/middlewares/tool-result-compaction.ts';

/** Regex to detect array truncation markers like "[... 5 more items removed]". */
const TRUNCATION_ARRAY_MARKER_REGEX = /\[\.\.\.\s+\d+\s+more items removed\]/;

/** Key used in objects to indicate truncation, e.g. { _truncated: "3 more keys removed" }. */
const TRUNCATION_OBJECT_MARKER_KEY = '_truncated';
...
export class ClaudeCacheStrategy extends AbstractCacheStrategy {
  ...
  #looksTruncated(value: unknown): boolean {
    ...
    return (
      COMPACTION_MARKER_PREFIXES.some((prefix) => s.includes(prefix)) ||
