...
export class MemoryReplayBuffer extends AbstractReplayBuffer {
  ...
  async endStream(streamId: string, error?: string, responseId?: string): Promise<void> {
    const stream = this.streams.get(streamId);
    if (!stream) return;
    ...
    const signalResponseId = responseId ?? stream.metadata.responseId;
    const msg: StreamEventMessage = error
      ? { type: 'error', error, responseId: signalResponseId }
      : { type: 'end', responseId: signalResponseId };
    for (const listener of stream.listeners) {
      listener(msg);
    }
  }
