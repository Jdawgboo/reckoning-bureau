...
export abstract class AbstractStorageAdapter {
  ...
  /** Whether this adapter supports search operations */
  get searchable(): boolean {
    return false;
  }

  /**
   * Whether this adapter participates in a no-path (fan-out) search.
   * Orthogonal to `searchable`: an adapter can be fully searchable when targeted
   * by an explicit path yet excluded from the default fan-out, so a high-volume
   * branch (e.g. runtime logs) never floods — or, under the result cap, evicts —
   * results from a branch-agnostic search. Default `true`; override to `false`
   * to require callers to name the branch explicitly.
   */
  get includeInDefaultSearch(): boolean {
    return true;
  }
  ...
  // ============================================================================
  // Search (optional — override in searchable adapters)
  // ============================================================================

  /**
   * Search file contents for a regex pattern.
   * Only called if `searchable` returns true.
   */
  async search(
    _pattern: string,
