...
/**
 * Input schema for the `deep_web_search` tool used by the deep-research
 * sub-agent. See `web-search-fallback.schema.ts` for the analogous main-agent
 * schema and the rationale for the standalone file.
 */
export const deepWebSearchParamsSchema = z
  .object({
    query: z.string().describe('Complete search query. Include ALL relevant details and context.'),
    start_time: z
      .string()
      ...
      .describe(
        'Optional RFC 3339 / ISO 8601 timestamp marking the EARLIER (older) edge of the date range ' +
          ...
          'Must be paired with end_time. For relative ranges, call getCurrentTime first.',
      ),
    end_time: z
      .string()
      .datetime({ offset: true })
...
export type DeepWebSearchParams = z.infer<typeof deepWebSearchParamsSchema>;
