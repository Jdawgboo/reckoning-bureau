...
import type { MCPClient } from '../../../services/mcp/mcp-client.ts';
...
export class MCPToolModel extends ToolModel<Record<string, unknown>> {
  ...
  constructor(config: {
    ...
    // Composio MCP schemas may ship `type: "array"` params with no `items`,
    // which Gemini/Vertex rejects (400 INVALID_ARGUMENT), failing the whole
    // tool list. Normalize once and use the sanitized schema everywhere it
    // reaches the model — including `getParameters()` (`rawInputSchema`).
    const sanitizedSchema = ensureArrayItems(config.tool.inputSchema as JSONSchema7);
    const wrappedSchema = jsonSchema(sanitizedSchema) as JSONSchema7;
    ...
    this.rawInputSchema = sanitizedSchema;
  }
