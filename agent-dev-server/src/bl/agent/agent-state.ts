...
import type { PreviewMessageT } from '../../types.ts';
import type { RequestContext } from '../../context/index.ts';
...
export function getSessionKey(state: BaseAgentState<unknown, unknown>): string | undefined {
  return state.getApp<DevServerAppState>()?.context?.getSessionKey();
}
