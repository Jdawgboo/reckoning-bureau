...
type BedrockMantleProvider = ReturnType<typeof createBedrockMantle>;
...
export class ModelProviderService implements ModelProvider {
  ...
  private bedrockMantleProvider: BedrockMantleProvider | null = null;
  ...
  getBedrockMantleProvider(): BedrockMantleProvider {
    if (!this.bedrockMantleProvider) {
      this.bedrockMantleProvider = this.config.baseUrl
        ? createBedrockMantle({
            baseURL: `${this.config.baseUrl}/amazon-bedrock-mantle/openai/v1`,
            apiKey: 'gateway',
            headers: this.gatewayHeaders,
          })
        : createBedrockMantle({
            baseURL: `https://bedrock-mantle.${process.env.AWS_REGION ?? 'us-east-1'}.api.aws/openai/v1`,
            apiKey: process.env.AWS_BEDROCK_API_KEY ?? '',
          });
    }
    return this.bedrockMantleProvider;
  }
  ...
  async getModel(modelName: string): Promise<LanguageModelV3> {
    ...
    switch (provider) {
      ...
      case 'amazon-bedrock-mantle': {
        return this.getBedrockMantleProvider().responses(modelName);
      }
  ...
  updateConfig(config: Partial<ModelProviderConfig>): void {
    ...
    this.bedrockMantleProvider = null;
