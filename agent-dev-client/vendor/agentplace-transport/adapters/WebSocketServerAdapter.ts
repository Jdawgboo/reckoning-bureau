...
export class WebSocketServerAdapter implements ITransport {
  ...
  constructor(ws: WebSocketLike, options: WebSocketServerAdapterOptions = {}) {
    ...
    ws.on('close', (code?: number, reason?: Buffer | string) => {
      ...
      if (wasConnected) {
        // Transport-layer signal only: warn for true server-side errors (1011 =
        // server failure) and TLS failures (1015). Codes 1005 (no close frame)
        // 1006 (abnormal network drop) and 4000-range (app-defined: stale
        // suspend/resume, heartbeat timeout, auth gates) are normal browser/
        // mobile/agent lifecycle. The owning service decides whether the
        // application context (e.g. an in-flight stream interrupted) warrants
        // a warn — the adapter doesn't have that context.
        if (code === 1011 || code === 1015) {
          console.warn(
            `[WebSocketServerAdapter] Connection closed with server-side error: code=${code}, reason=${reasonStr}`,
          );
        } else {
          console.log(
  ...
  #startHeartbeat(): void {
    ...
    const pingTimer = setInterval(() => {
      ...
      this.#unrefTimer(pongTimer);
      this.#pongTimer = pongTimer;
    }, pingInterval);
    this.#unrefTimer(pingTimer);
    this.#pingTimer = pingTimer;
  }

  /**
   * Detach a heartbeat timer from the event loop. The heartbeat must never be
   * the sole reason the process stays alive — in production the listening socket
   * keeps the server up, while in tests there is no such anchor, so a referenced
   * timer would keep `node --test` from exiting. No-op in environments where
   * timers are numeric (browser), which the server adapter never runs in.
   */
  #unrefTimer(timer: ReturnType<typeof setInterval> | ReturnType<typeof setTimeout>): void {
  ...
  send(packet: Packet, _transfer?: TransferableItem[]): void {
    if (!this.isConnected) {
      // Benign race: the underlying socket closed between the caller's enqueue and
      // this send. All RpcPeer call paths handle the resulting `void` return via
      // .catch()/try-catch. Logged at info — not actionable as an error, but kept
      // in CloudWatch so storms (e.g. a backgrounded tab dropping pongs and
      // cycling reconnects) remain diagnosable.
      console.info('[WebSocketServerAdapter] Cannot send: not connected');
      return;
    }
