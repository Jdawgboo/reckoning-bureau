  ...
  ({ className, variant = 'outline', size = 'icon', ...props }, ref) => {
    ...
    return (
      <Button
        ...
        size={size}
        className={cn(
          'absolute h-8 w-8 rounded-sm bg-glass hover:bg-primary',
      ...
      >
        ...
        <span className="sr-only">{intl.formatMessage(messages.carouselPrevious)}</span>
      </Button>
    );
  },
);
  ...
  ({ className, variant = 'outline', size = 'icon', ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        <span className="sr-only">{intl.formatMessage(messages.carouselNext)}</span>
      </Button>
    );
  },
);
...
const CarouselDots = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn('embla__dots z-50 mt-4 flex items-center justify-center gap-4', className)}
        {...props}
      >
        {api?.scrollSnapList().map((_, index) => (
          <Button
            key={index}
            className={cn(
              'embla__dot h-1 w-1 rounded-full px-1 py-1',
              index === selectedIndex ? 'embla__dot--selected ' : 'bg-muted',
            )}
