...
export function DatePickerWithSelection({ onChange }: DatePickerProps) {
  const intl = useIntl();
        ...
        <div className="flex justify-between p-2 space-x-1">
          <Select onValueChange={handleYearChange} value={year.toString()}>
            <SelectTrigger className="w-full">
              <SelectValue placeholder={intl.formatMessage(messages.year)} />
            </SelectTrigger>
            <SelectContent>
              {years.map((y) => (
          ...
          <Select onValueChange={handleMonthChange} value={month.toString()}>
            <SelectTrigger className="w-[120px]">
              <SelectValue placeholder={intl.formatMessage(messages.month)} />
            </SelectTrigger>
            <SelectContent>
              {months.map((m, index) => (
                <SelectItem key={index} value={index.toString()}>
                  {intl.formatDate(m, { month: 'long' })}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
